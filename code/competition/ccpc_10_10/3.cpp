...
void work()
{
    ...
    sort(egs+1,egs+n);
    for(int i=1;i<n;i++){
        if(val[egs[i].u]>val[egs[i].v])
            swap(egs[i].v,egs[i].u);
        mp[find(egs[i].v)].push_back(find(egs[i].u));
