...
inline int qjudge(int di,int mi,int ai){
    int res = 1;
    while(mi){
        // cout<<mi<<' '<<res<<' '<<di<<endl;
        if(di>ai||res>ai) return 1;

        if(mi%2){
            if(ai/di<res) return 2;
            res *= di;
            mi--;
        }
