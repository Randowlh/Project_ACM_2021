...
int du[maxn],u[maxn],v[maxn],a[maxn],b[maxn];
void work()
{
    cin>>n>>m>>k;
    zkw.n=4100;
    ...
    zkw.t=4090;
    int cnt=510;
    // zkw.init(1100,1089,1090);
    for(int i=1;i<=n;i++)
        zkw.add_edge(i,4090,k,0);
    for(int i=1;i<=m;i++){
        cin>>u[i]>>v[i]>>a[i]>>b[i];
        zkw.add_edge(v[i],++cnt,1,0);
        zkw.add_edge(cnt,4090,1,b[i]);
        zkw.add_edge(cnt,u[i],1,a[i]);
        du[v[i]]++;
    }
    for(int i=1;i<=n;i++)
        if(du[i])
            zkw.add_edge(4089,i,du[i],0);
    zkw.cost_flow();
    cout<<zkw.ans<<endl;    
}
signed main()
